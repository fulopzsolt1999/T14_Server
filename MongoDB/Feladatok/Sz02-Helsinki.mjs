...
async function helsinkiExercise() {
   try {
      const client = await MongoClient.connect(url);
      const db = client.db("T14");
      const collection = db.collection("Helsinki");
      const helsinkiObject = helsinkiArrayToObject();

      /* await collection.deleteMany();
      await db.collection("Helsinki").insertMany(helsinkiObject); */

      const gymSwim = await db
         .collection("Helsinki")
         .find(
            {$or: [{sport: "torna"}, {sport: "uszas"}]},
            {projection: {_id: 0, sport: 1, event: 1}}
         )
         .sort({sport: 1})
         .toArray();

      const rank123 = await db
         .collection("Helsinki")
         .find({rank: {$lte: 3}}, {projection: {_id: 0, event: 1}})
         .sort({event: 1})
         .toArray();

      const biggestTeamSize = await db
         .collection("Helsinki")
         .find()
         .sort({teamSize: -1})
         .limit(1)
      ...
      const soloGold = await db
         .collection("Helsinki")
         .find({$and: [{teamSize: 1}, {rank: 1}]}, {projection: {_id: 0, sport: 1}})
         .sort({sport: 1})
         .toArray();

      logAllResults(gymSwim, rank123, biggestTeamSize, soloGold);
...
function helsinkiArrayToObject() {
   const helsinkiArray = [
   ...
   const helsinkiObject = [];
   for (const line of helsinkiArray) {
      helsinkiObject.push({
         rank: Number(line.split(" ")[0]),
         teamSize: Number(line.split(" ")[1]),
         sport: line.split(" ")[2],
         event: line.split(" ")[3],
      });
   }
   return helsinkiObject;
}

function logAllResults(gymSwim, rank123, biggestTeamSize, soloGold) {
   console.log(gymSwim);
   console.log(rank123);
   console.log(biggestTeamSize);
   console.log(soloGold);
}
